...
|:-----|:-----|:-----|
|  **resid**  |  Yes  | Specifies the URL location of the HTML page for your add-in. The `resid` can be no more than 32 characters and must match an `id` attribute of a `Url` element in the `Resources` element.<br><br>**Important**: When implementing a `LaunchEvent` extension point in your manifest, ensure that the `resid` attribute of the extension point's [\<SourceLocation\>](customfunctionssourcelocation.md) element matches the `resid` value of the **\<Runtime\>** element. For example, if your runtime is defined as `<Runtime resid="WebViewRuntime.Url">`, specify `<SourceLocation resid="WebViewRuntime.Url"/>` in the `LaunchEvent` extension point. |
