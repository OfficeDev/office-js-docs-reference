...
| **HeaderName** | No | Specifies the internet header name used to identify a message on which the `OnMessageDecrypt` or `OnMessageReadWithCustomHeader` (preview) event occurs. The `Type` attribute must be set to `OnMessageDecrypt` or `OnMessageReadWithCustomHeader`. |
