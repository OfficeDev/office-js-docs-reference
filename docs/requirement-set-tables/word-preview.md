...
||[getBookmarkRange(name: string)](/javascript/api/word/word.document#getbookmarkrange-name-)|Gets a bookmark's range. Throws an error if the bookmark does not exist.|
...
||[getBookmarkRange(name: string)](/javascript/api/word/word.documentcreated#getbookmarkrange-name-)|Gets a bookmark's range. Throws an error if the bookmark does not exist.|
...
||[getItem(key: string)](/javascript/api/word/word.settingcollection#getitem-key-)|Gets a setting object by its key, which is case-sensitive. Throws an error if the setting does not exist.|
