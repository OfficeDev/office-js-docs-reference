...
The **\*-snippets.yaml** files in this repo contain samples for each application and the Common APIs (**office-snippets.yaml**). Each YAML key is the UID of a field in the documentation. The keys follow the format `<Application>.<ClassName>#<fieldName>:member<overloadNumber(methods only)>`. Add your sample code snippet as the value under that key. Use the following Git mechanisms to submit changes nad pull requests to the repo.
...
* Limit the scope. Sample code that focuses on a small scenario or API usage helps readers understand how that API is used. (Larger samples should go to [Script Lab](https://github.com/OfficeDev/office-js-snippets) or the [Code Samples repo](https://github.com/OfficeDev/office-js-snippets).)
